"""Legacy demo-dashboard ORM models.

These predate the simulator cockpit and the agentic audit log. They back the
older recruiter-facing visualization dashboard (benchmark/task/model metrics and
the ``Demo*`` synthetic visual-demo rows). Kept for backward compatibility; new
work should target ``models_audit`` (agentic loop) or ``models_simulator``
(replay cockpit). See AUDIT_AND_PLAN.md §3.
